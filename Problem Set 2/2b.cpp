...
int main()
{
    ...
    for (int i = 0; i < lines; i++)
    {
        ...
        // Keep popping front of the right half queue and push it to the end of the left half queue
        // until they have the similar size (within 1 size difference)
        if (q2.size() > q1.size())
        {
            //  iterate over their differences divide by 2.0 (because it could be a decimal from division), then use ceil fucntion to round down the decimal
            for (int j = 0; j < ceil((q2.size() - q1.size()) / 2.0); j++)
            {
                int temp = q2.front();
                q2.pop_front();
                q1.push_back(temp);
        ...
        // Keep popping back of the left half queue and push it to the front of the right half queue
        // until they have the similar size (within 1 size difference)
        // repeat same but opposite from the if statement above
        else if ((q1.size() - q2.size()) > 1)
        {
            for (int k = 0; k < floor((q1.size() - q2.size()) / 2.0); k++)
            {
                int temp = q1.back();
                q1.pop_back();
                q2.push_front(temp);
